...
private: //common
  ...
  void setMouseWheelState(const int_fast32_t amplitude) {
    mMouseWheelStatus += amplitude;
  }
...
private: //UNIX
  ...
  void handleEvents(const XEvent *const pevent) {
    ...
    case ButtonRelease: {
      ...
      do {
        mMousePosX = event.xmotion.x; mMousePosY = event.xmotion.y;
        if (mMousePosX < 0 || mMousePosY < 0 || mMousePosX >= (int)mDataWidth || mMousePosY >= (int)mDataHeight) {
          mMousePosX = -1;
          mMousePosY = -1;
        }
        switch (event.xbutton.button) {
        case 1: setMouseButtonState(1, false); break;
        case 3: setMouseButtonState(2, false); break;
        case 2: setMouseButtonState(3, false); break;
  ...
  void mapWindow() {
    ...
    do { // Wait for the window to be visible.
      ...
      if (attr.map_state != IsViewable) { XSync(dpy, 0); sleep(10); }
  ...
  void constructImpl(const unsigned int dimw, const unsigned int dimh, const char *const title = 0) {
    ...
    if (!dpy) {
      ...
      if (x11.mBitDepth != 8 && x11.mBitDepth != 16 &&
        x11.mBitDepth != 24 && x11.mBitDepth != 32) {
        std::cerr << "Invalid screen mode detected (only 8, 16, 24 and 32 bits modes are managed)." << std::endl;
        exit(1);
      }
    ...
    if (XShmQueryExtension(dpy)) {
      ...
      mXImage = XShmCreateImage(dpy, DefaultVisual(dpy, DefaultScreen(dpy)), x11.mBitDepth, ZPixmap, 0, mShmInfo, mDataWidth, mDataHeight);
      if (!mXImage) {
        delete mShmInfo; mShmInfo = 0;
      } else {
        ...
        if (mShmInfo->shmid == -1) {
          ...
          if (mShmInfo->shmaddr == (char*)-1) {
            ...
            if (!x11.mShmEnabled) {
              shmdt(mShmInfo->shmaddr); shmctl(mShmInfo->shmid, IPC_RMID, 0);
              XDestroyImage(mXImage);
              delete mShmInfo;
              mShmInfo = 0;
            }
  ...
  void render(const unsigned char* data, int width, int height) {
    ...
    if (x11.mIsBigEndian == isBigEndian()) {
      for (int xy = width * height; xy > 0; --xy) {
        *(ndata++) = ((unsigned char)*(data) << 16) | ((unsigned char)*(data + 1) << 8) | (unsigned char)*(data + 2);
        data += 3;
      }
    } else {
      for (int xy = width * height; xy > 0; --xy) {
        *(ndata++) = ((unsigned char)*(data) << 24) | ((unsigned char)*(data + 1) << 16) | ((unsigned char)*(data + 2) << 8);
        data += 3;
      }
    }
...
private: //WINDOWS
...
#endif
    //TODO: Create function in Display class to handle event. Improve encapsulation.
    switch (msg) {
    case WM_CLOSE:
...
#endif
      if (disp->mMousePosX < 0 || disp->mMousePosY < 0 || disp->mMousePosX >= (int)disp->mDataWidth || disp->mMousePosY >= (int)disp->mDataHeight) {
        disp->mMousePosX = disp->mMousePosY = -1;
      }
  ...
  static DWORD WINAPI mEventThread(void* arg) {
    ...
    RECT rect;
    rect.left = rect.top = 0; rect.right = (LONG)disp->mDataWidth - 1; rect.bottom = (LONG)disp->mDataHeight - 1;
    ...
    disp->mWindowHandle = CreateWindowA("MDICLIENT", title ? title : " ",
      WS_OVERLAPPEDWINDOW | (disp->mIsHidden ? 0 : WS_VISIBLE),
      CW_USEDEFAULT,
      CW_USEDEFAULT,
      disp->mDataWidth + 2 * border1,
      disp->mDataHeight + border1 + border2,
      0,
      0,
      0,
      &disp->mClientCreateStruct);
    if (!disp->mIsHidden) {
  ...
  void updateWindowPosition() {
    if (mIsHidden) {
      ...
      RECT rect;
      rect.left = rect.top = 0; rect.right = (LONG)mDataWidth - 1; rect.bottom = (LONG)mDataHeight - 1;
      AdjustWindowRect(&rect, WS_CAPTION | WS_SYSMENU | WS_THICKFRAME | WS_MINIMIZEBOX | WS_MAXIMIZEBOX, false);
  ...
  void constructImpl(const unsigned int dimw, const unsigned int dimh, const char *const title = 0) {
    ...
    mmEventThreadHandle = CreateThread(0, 0, mEventThread, arg, 0, 0); //TODO: Use std::thread instead.
...
public: //WINDOWS
  ...
  void move(const int posx, const int posy) {
    if (mWindowPosX != posx || mWindowPosY != posy) {
      RECT rect;
      rect.left = rect.top = 0; rect.right = (LONG)mWindowWidth - 1; rect.bottom = (LONG)mWindowHeight - 1;
      AdjustWindowRect(&rect, WS_CAPTION | WS_SYSMENU | WS_THICKFRAME | WS_MINIMIZEBOX | WS_MAXIMIZEBOX, false);
      const int border1 = (int)((rect.right - rect.left + 1 - mDataWidth) / 2);
      const int border2 = (int)(rect.bottom - rect.top + 1 - mDataHeight - border1);
  ...
  void paint() {
    ...
    SetDIBitsToDevice(mDeviceContextHandle, 0, 0, mDataWidth, mDataHeight, 0, 0, 0, mDataHeight, mPixels, &mBitmapInfo, DIB_RGB_COLORS);
